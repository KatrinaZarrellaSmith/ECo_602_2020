...
Next I will explore the data visually. Since the points will overlap, I will choose a color with transparacy to display the data more clearly:
```{r color}
...
```{r scatterplot, fig.height=6, fig.width=9}
...
plot(dat_delomys$body_mass, dat_delomys$body_length,
     ...
     xlab = 'Body Mass (g)',
     ylab = 'Body Length (mm)',
     col = my_colors, # Color to show two species
     pch = 3) # Change data point symbol

```

Note that this relationship does not seem linear. Istead it appears to curve.
...
Note that this histogram is not normally distributed and has a right-skew in the data. This also matches with the results of the Shapiro-Wilk normality test providing evidence that body length was not sampled from a normally distributed population.
...
Note, the body mass between D. dorsalis and D. sublineatus do not appear to be dramatically different.

## Sex {.tabset .tabset-pills}
```{r box_sex, fig.height=6, fig.width=5}
...
Note, the body mass between females and males of Delomys spp. do not appear to be dramatically different.

## Species and Sex {.tabset .tabset-pills}
```{r box_species_sex, fig.height=7, fig.width=9}
...
Next I will build a linear model:
```{r linear}
...
I would select models 4 and 5. There is not much of a difference in AIC between model 4 and model 5 and the next lowest AIC is more than 5 units away from the second lowest AIC. Because the joint effect of sex and species was not statistically significant in model 5, these models have somewhat similar results. They are a reasonable fit and lead to coefficients that are reasonably close in values. Ecological explanations for the main effects by themselves can be done with model 4: body mass varying by sex and body mass varying by species. Additonally, by disregarding the interaction factor, the model is simpler. Discussing model 5 with an interaction term would only be necessary if the interaction was ecologically significant.
